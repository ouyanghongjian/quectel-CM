...
static void* udhcpc_thread_function(void* arg) {
	...
	ptr = strtok_r(udhcpc_cmd, " ", &p);
	while(ptr != NULL && i < (MAX_LEN_CMD - 1) )
	{
		argv[i++] = ptr;
		ptr = strtok_r(NULL, " ", &p);
	}
...
#else
#ifdef USE_DHCLIENT
    if (dhclient_alive && (pid_dhclient != -1)) {
        //system("killall dhclient");
		snprintf(shell_cmd, sizeof(shell_cmd) - 1, "kill -9 %d", pid_dhclient); 
		ql_system(shell_cmd);
        dhclient_alive = 0;
		pid_dhclient = -1;
    }
